...
public class TimeConstraintStateHandler extends AbstractEvaluatorHandler
{
	protected static Logger LOGGER = Logger.getLogger(TimeConstraintStateHandler.class);
	...
	{
		ServiceTermStateType serviceTermState = ServiceTermStateHelper.instance().getTimeConstraint(pAgreementProperties);
	...
	public void evaluate(AgreementPropertiesDocument pAgreementProperties)
	{
		...
		{
			LOGGER.info("[" + pAgreementProperties.getAgreementProperties().getAgreementId() + "] time constraint state is 'NOT_READY'");
		...
		{
			LOGGER.info("[" + pAgreementProperties.getAgreementProperties().getAgreementId() + "] time constraint state is 'COMPLETED'");
		...
		{
			LOGGER.info("[" + pAgreementProperties.getAgreementProperties().getAgreementId() + "] time constraint state is 'READY'");
			setServiceTermState(serviceTermState, ServiceTermStateDefinition.READY);
		} else
		{
			LOGGER.error("[" + pAgreementProperties.getAgreementProperties().getAgreementId() + "] time constraint state could not be determined.");
